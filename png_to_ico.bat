...
IF EXIST %argPath%\* (
	...
	FOR %%f IN (%argPath%\*.png %argPath%\*.bmp %argPath%\*.gif %argPath%\*.jpg %argPath%\*.jpeg %argPath%\*.svg) DO (
		...
		"%directoryPath%ImageMagick\magick.exe" "%%f" -resize 256x256^> -background none -gravity center -extent 256x256 -define icon:auto-resize=256,128,96,64,48,32,24,16 "%%~df%%~pf%%~nf.ico"
	)
REM If first argument is a file
	...
	FOR %%f IN (%argPath%) DO (
		...
		ECHO - %%~nf%%~xf
		REM Convert file to multi-resolution ICO
		"%directoryPath%ImageMagick\magick.exe" %argPath% -resize 256x256^> -background none -gravity center -extent 256x256 -define icon:auto-resize=256,128,96,64,48,32,24,16 "%%~df%%~pf%%~nf.ico"
	)
)